...
//类的实例化：
//	用类类型创建对象的过程，称为类的实例化
//	1：类只是模型一样的东西，限定了类有那些成员，定义一个类并没有分配实际的内存空间来存储它。
//	2：一个类可以实例化出多个对象，实例化出的对象占用实际的物理空间，存储类成员变量。
//	3：比方类实例化出的对象就像现实中使用建筑设计图创建出房子，类就像设计图纸，之设计出需要什么东西，但是并没有实体的建筑存在，
//	   同样类也只是一个设计实例化出的对象才能存储实际的数据，占用物理空间。
//封装：（本质是一种管理）
//	C语言不封装，也没有办法封装，他的数据与方法是分离的
//	C++必须规范的使用函数访问数据，不能直接访问数据
//	封装是：将数据和操作数据的方法进行有机结合，隐藏对象的属性和实现细节，仅 对外公开接口 来和对象交互的
#include "Person.h"
